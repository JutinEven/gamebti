...
class S3SyncStorage:
    ...
    def _get_client(self):
        if self._client is None:
            endpoint = self.endpoint_url or os.environ.get("S3_ENDPOINT_URL", "")
            if not endpoint:
                logger.error("未配置存储端点：请设置 S3_ENDPOINT_URL 环境变量或传入 endpoint_url")
                raise ValueError("未配置存储端点：请设置 S3_ENDPOINT_URL 或传入 endpoint_url")

            client = boto3.client(
                "s3",
                endpoint_url=endpoint,
                aws_access_key_id=self.access_key or os.environ.get("S3_ACCESS_KEY", ""),
                aws_secret_access_key=self.secret_key or os.environ.get("S3_SECRET_KEY", ""),
                region_name=self.region or os.environ.get("S3_REGION", "cn-beijing"),
            )
    ...
    def generate_presigned_url(self, *, key: str, bucket: Optional[str] = None, expire_time: int = 1800) -> str:
        """通过 S3 Proxy 生成签名 URL。需要 S3_ENDPOINT_URL 环境变量。"""
        import json
        ...
        # 如果环境变量中有 storage token，注入头
        storage_token = os.environ.get("S3_STORAGE_TOKEN", "")
        if storage_token:
